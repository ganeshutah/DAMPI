...
int ADLB_End_batch_put()
    ...
    These two functions can enhance performance if an app calls them before and
...
int ADLB_Put(void *work_buf, int work_len, int reserve_rank, int answer_rank,
    ...
    ADLB_PUT(work_buf, work_len, reserve_rank, answer_rank, work_type, work_prio, ierr)

    Put work into the queue.
    work_buf points to the work itself.
    work_len is the length of the work in bytes.
    reserve_rank can be -1 (wild card) if the work is not reserved for any particular
    rank.  If >= 0 it reserves the work for the specified rank.
